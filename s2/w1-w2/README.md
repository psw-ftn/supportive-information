...
Većina kartica u S2 podrazumevaju da se naprave agregati objekata (skupina objekata koji se koriste zajedno da ispune deo slučaja korišćenja) i da se u njih prebaci veći deo poslovne logike. Da bismo ovo postigli, potrebno je da:

1. Analiziramo tekst naše kartice i ostalih kartica koje se tiču istog većeg koncepta u okviru jednog modula. Taj "veći koncept" će biti koren agregata. Cilj je da pobrojimo klase domenskog sloja koje su potrebne da podrže funkcionalnosti tražene od tog "većeg koncepta" i koje će biti deo agregata kojim upravlja koren agregata.
2. Klasifikacija svake klase kao "koren agregata", "entitet" ili "vrednosni objekat", gde u opštem slučaju važi:
   1. Koren agregata je:
      1. Entitet, što znači da ima identifikator i životni ciklus.
      2. Modeluje "veći koncept" koji ujedno definiše naziv agregata i tog korena agregata.
      3. Može da bude samostalna klasa (u kom slučaju imamo agregat koji se sastoji od jednog objekta) ili da sadrži asocijacije ka drugim entitetima i vrednosnim objektima.
      4. Koren agregata je zadužen za održavanje validnosti i konzistentnosti čitavog agregata (dakle, da se svi objekti agregata kolektivno nalaze u validnom stanju).
      5. Brisanjem korena agregata bismo trebali smisleno da obrišemo i sve povezane objekte.
      6. Servisi interaguju (pozivaju metode) isključivo od korena agregata kada žele nešto od kompletnog agregata.<br>
   _Primer_: Tura je koren agregata i ona sadrži više klučnih tačaka. Brisanjem Ture ima smisla da obrišemo i njene ključne tačke. Problem nastaje kod slučaja korišćenja "prikaži javne ključne tačke". Ako bismo tretirali ključne tačke samo kao entitet u okviru Ture, morali bismo da učitamo sve Ture, da ih pitamo koje javne tačke imaju i da njih vratimo. Druga opcija je da postoji odvojeni koncept "javne ključne tačke", gde bi javna ključna tačka bila koren svog (jednoobjektnog) agregata. Ova opcija ima smisla sa aspekta da javna ključna tačka neće imati tajnu, dok će svaka Tura koja koristi tu ključnu tačku definisati svoju tajnu za nju. Kada želimo da dodamo javnu ključnu tačku u Turu, mi bismo napravili kopiju te tačke za potrebe te Ture i dozvolili korisniku unos tajne specifične za tu Turu.
   3. Entitet može da bude deo agregata, a da nije njegov koren, kada je jasno podređen drugom entitetu i kada ima svoj životni ciklus. Kada objekat može da se menja i potreban mu je identifikator (npr. za potrebe pronalaska datog objekta u listi objekata), kandidat je za entitet.
   4. Vrednosni objekat sadrži kohezivne podatke i logiku koja radi nad njima. Koristimo ga kada klasa nema jasan životan ciklus i jasnu potrebu za identifikatorom. Ovo ne znači da polja te klase moraju da budu nepromenljiva (engl. immutable).<br>
   _Primer_: "Poruka" vezana za "problem sa turom" je deo problema kom pripada. Ako poruka ne treba da se menja, ovaj koncept je očigledan vrednosni objekat. Ako je potrebno omogućiti izmenu poruke, možda će nam identifikator olakšati tu funkcionalnost kako bismo lakše locirali poruku koju treba izmeniti (gde bi alternativa bila da "problem sa turom" pronađe staru poruku spram `CreationTime` i `UserId`).
